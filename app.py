...
worksheet = None
try:
    creds_base64 = st.secrets.get("GCP_CREDENTIALS_BASE64", "")
    if creds_base64 and creds_base64 != "YOUR_GCP_CREDENTIALS_BASE64_HERE":
    ...
    # Google Sheets is optional - app will work without it
    worksheet = None

# --- AI Model Setup ---
try:
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
    # Use gemini-flash-latest (free tier supported model)
...
if prompt := st.chat_input("What are your interests and skills?"):
    ...
    try:
        ...
        if prompt.lower().strip() in greetings:
            ...
            # 2. Get the AI's detailed text response with retry logic for rate limits
            max_retries = 3
            retry_delay = 5
                ...
                try:
                    ...
                    if "429" in error_str or "quota" in error_str.lower() or "rate" in error_str.lower():
                        if attempt < max_retries - 1:
                            ...
                            raise Exception(f"Rate limit exceeded. Please try again in a few minutes. Error: {e}")
                    else:
                        raise e
    ...
    if worksheet:
        try:
            ...
            # Silently fail if Sheets is not available
            pass
